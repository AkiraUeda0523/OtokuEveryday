...
class DBRegisterModel {
    ...
    func regestarDocumentID() {// 二発入る現象は何故⚠️fb更新時（ジオ取れた時）に倍々になってる　　グローバルのボックスが原因か！　※fb更新時メソッドも実行
        ...
        Firestore.firestore().collection("map_ addresses").getDocuments { [self] snapShot, error in
    ...
    func errorID() {// ⚠️⚠️⚠️手入力の際は文字列ではなくIntに変更必須‼️
        ...
        Firestore.firestore().collection("map_ addresses").getDocuments { snapShot, error in
